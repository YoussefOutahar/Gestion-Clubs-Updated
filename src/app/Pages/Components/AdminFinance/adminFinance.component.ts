...
export class AdminFinanceComponent implements OnInit {
  ...
  async fetchData() {
    ...
    this.totalBudget = await this.clubsService.getTotalBudgetByYear(
      new Date().getFullYear()
    );
    this.totalSuppBudget =
      await this.clubsService.getTotalSupplementaryBudget();

    this.cardList = [
      {
        name: 'Total Budget',
        amount: `${this.totalBudget} DH`,
        icon: 'attach_money',
      },
      {
  ...
  async fetchClubData() {
    try {
      ...
      for (const club of allClubs) {
        const total_supp_budget =
          await this.clubsService.getTotalSupplementaryBudgetByClub(club.id!);
        const total_earnings =
          await this.clubsService.getTotalSuppEarningsByClub(club.id!);
